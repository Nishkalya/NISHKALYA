...
export default function AdminPerformanceDashboard() {
  ...
  const getBrowserIcon = (browserName: string) => {
    switch (browserName) {
      ...
      default:
        return <Globe size={14} className="text-slate-500 font-sans" />;
    }
  };
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono">Avg Initial Load</span>
            <div className="p-1.5 bg-[#238636]/10 border border-[#238636]/20 rounded-lg text-[#2ea44f]"><Gauge size={14} /></div>
          </div>
          <div className="flex items-baseline gap-2">
            <div className="text-2xl md:text-3xl font-black text-slate-900">{avgPageLoad || 'N/A'} <span className="text-xs font-light text-slate-500">ms</span></div>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono">Avg Transition</span>
            <div className="p-1.5 bg-[#58a6ff]/10 border border-[#58a6ff]/20 rounded-lg text-blue-600"><Layers size={14} /></div>
          </div>
          <div className="flex items-baseline gap-2">
            <div className="text-2xl md:text-3xl font-black text-slate-900">{avgTransition || 'N/A'} <span className="text-xs font-light text-slate-500">ms</span></div>
          </div>
          <div className="text-[9px] text-slate-500 mt-2 flex items-center gap-1">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono">High Latency Peak</span>
            <div className="p-1.5 bg-rose-500/10 border border-rose-500/20 rounded-lg text-rose-400"><Clock size={14} /></div>
          </div>
          <div className="flex items-baseline gap-2">
            <div className="text-2xl md:text-3xl font-black text-slate-900">{maxLatency || 'N/A'} <span className="text-xs font-light text-slate-500">ms</span></div>
          </div>
          <div className="text-[9px] text-slate-500 mt-2 flex items-center gap-1">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono">Telemetry Nodes</span>
            <div className="p-1.5 bg-violet-500/10 border border-violet-500/20 rounded-lg text-violet-400"><Database size={14} /></div>
          </div>
          <div className="flex items-baseline gap-2">
            <div className="text-2xl md:text-3xl font-black text-slate-900">{totalCount} <span className="text-xs font-light text-slate-500">logs</span></div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6 flex flex-col">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col justify-between flex-1">
            <div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5 font-mono">
                  ...
                  <span className="text-slate-900">{excellentCount} logs ({excellentPct}%)</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5 font-mono">
                  ...
                  <span className="text-slate-900">{goodCount} logs ({goodPct}%)</span>
                </div>
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5 font-mono">
                  ...
                  <span className="text-slate-900">{slowCount} logs ({slowPct}%)</span>
                </div>
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            ...
            <div className="mt-8 pt-6 border-t border-slate-200 flex items-center justify-between">
              <span className="text-[10px] text-slate-500 font-mono">Admin Control:</span>
              {!isConfirmPurgeOpen ? (
                <button 
                  onClick={() => setIsConfirmPurgeOpen(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold text-rose-400 hover:text-slate-900 border border-rose-500/20 hover:bg-rose-600/10 rounded-lg admin-glow transition-all uppercase tracking-wider"
                >
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="px-2.5 py-1.5 bg-rose-600 text-slate-900 font-bold rounded-lg text-[10px] hover:bg-rose-700 disabled:opacity-50 font-sans"
                  >
                    {isActionPending ? 'Purging...' : 'Confirm'}
                  </button>
                  <button 
                    onClick={() => setIsConfirmPurgeOpen(false)}
                    className="px-2.5 py-1.5 bg-slate-100 text-slate-500 font-bold rounded-lg text-[10px] hover:text-slate-900 font-sans"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
            <h3 className="text-sm font-bold text-slate-900 mb-2 flex items-center gap-2 font-sans">
              <Monitor size={16} className="text-blue-600" /> Browser Latency
            </h3>
            <p className="text-xs text-slate-500 font-light leading-relaxed mb-5 font-sans">
              Average request latency and routing speed categorized by browser agent.
            </p>

            <div className="space-y-4">
              {isLoading ? (
                <div className="py-8 text-center text-[11px] text-slate-500 font-mono animate-pulse">
                  Analyzing user-agents...
                </div>
              ) : browserStats.length === 0 ? (
                <div className="py-8 text-center text-xs text-slate-500 font-light font-sans">
                ...
                browserStats.map((stat) => {
                  ...
                  return (
                    <div key={stat.browser} className="p-3 rounded-xl bg-slate-50 border border-slate-200 hover:border-slate-200 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          {getBrowserIcon(stat.browser)}
                          <span className="text-xs font-bold text-slate-900 font-mono">{stat.browser}</span>
                          <span className="text-[9px] text-slate-500 font-sans">({stat.count} node{stat.count > 1 ? 's' : ''})</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between text-[9px] text-slate-500 font-mono mb-2.5">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h3 className="text-sm font-bold text-slate-900 mb-1">Diagnostic Log Console</h3>
              <p className="text-[10px] text-slate-500">Recent loading and navigation timing signals recorded.</p>
            </div>
          ...
          <div className="overflow-hidden border border-slate-200 rounded-xl bg-slate-50 relative">
            <div className="max-h-[385px] overflow-y-auto divide-y divide-slate-200 [scrollbar-width:thin] [scrollbar-color:#30363d_transparent]">
              {isLoading ? (
                <div className="py-20 text-center flex flex-col items-center justify-center gap-3">
                  <RefreshCw size={24} className="text-blue-600 animate-spin" />
                  <span className="text-xs text-slate-500 font-mono">Connecting to real-time firestore stream...</span>
                </div>
              ) : filteredLogs.length === 0 ? (
                <div className="py-20 text-center">
                  <Activity size={24} className="text-zinc-600 mx-auto mb-3" />
                  <span className="text-xs text-slate-500">Empty stream. No telemetry received yet.</span>
                </div>
              ) : (
                filteredLogs.map((log) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 cursor-pointer hover:bg-white transition-colors gap-3 ${selectedLog?.id === log.id ? 'bg-[#1f242c]' : ''}`}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        {log.type === 'pageload' ? (
                          ...
                          <div className="w-8 h-8 rounded-lg bg-[#58a6ff]/10 border border-[#58a6ff]/20 flex items-center justify-center text-blue-600 shrink-0">
                            <Layers size={14} />
                          </div>
                        )}
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className="text-xs font-bold text-slate-900 font-mono break-all leading-none">{log.page === '/' ? 'Home View' : log.page}</span>
                            <span className="text-[8px] text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded font-mono uppercase tracking-wide">
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-4">
                        ...
                        <button 
                          ...
                          className="p-1.5 text-slate-500 hover:text-red-400 bg-transparent hover:bg-red-500/10 rounded-lg transition-colors border border-transparent hover:border-red-500/25"
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
      ...
      <AnimatePresence>
        {selectedLog && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="p-5 bg-slate-50 border border-slate-200 rounded-2xl relative"
          >
            <button 
            ...
            <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest mb-4 font-mono">Performance Event Metadata</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2.5 text-xs">
                <div>
                  <span className="text-slate-500 font-mono block mb-0.5">Payload ID:</span>
                  <span className="text-slate-900 font-mono break-all bg-white px-2 py-1 rounded border border-slate-200 inline-block">{selectedLog.id}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-mono block mb-0.5">Measurement Type:</span>
                  <span className="text-slate-900 font-serif">{selectedLog.type === 'pageload' ? 'Complete static page download latency' : 'Client-side SPA React view translation'}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-mono block">Action Destination:</span>
                  <span className="text-blue-600 font-bold">"{selectedLog.page}"</span>
                </div>
              </div>
              <div className="space-y-2.5 text-xs">
                <div>
                  <span className="text-slate-500 font-mono block mb-0.5">Report Time:</span>
                  <span className="text-slate-900 font-mono">{selectedLog.createdAt?.toDate ? selectedLog.createdAt.toDate().toUTCString() : 'Just recorded'}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-mono block mb-0.5">Device Fingerprint:</span>
                  <span className="text-slate-500 text-[10px] block leading-relaxed max-w-md font-mono select-all bg-white px-2 py-1.5 rounded border border-slate-200 truncate" title={selectedLog.userAgent}>
